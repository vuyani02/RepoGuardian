...
using FullStackProject.Models.TokenAuth;
...
{
    public abstract class FullStackProjectWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static FullStackProjectWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(FullStackProjectWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<FullStackProjectDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<FullStackProjectDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<FullStackProjectDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<FullStackProjectDbContext, Task<T>> func)
        {
            T result;
